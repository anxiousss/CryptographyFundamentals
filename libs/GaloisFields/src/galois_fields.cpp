...
namespace galois_fields {
    ...
    void GaloisField::print_element(const std::vector<std::byte>& element) {
        for (int i = 15; i > -1; --i) {
            auto bit = bits_functions::get_younger_bit(element[(15 - i) / 8], i % 8);
            if (bit == 1 && i != 0)
                std::cout << "x^" << i << " + ";
            if (i == 0)
                std::cout << bit;
    ...
    std::pair<std::byte, std::byte> GaloisField::divide(const std::vector<std::byte> &a, const std::byte &b) {
        uint16_t poly1 = bits_functions::bytes_to_uint16(a);
        uint16_t poly2 = bits_functions::bytes_to_uint16({b});

        auto [quotient, remainder] = bits_functions::divide_with_quotient(poly1, poly2);

        std::vector<std::byte> quotient_bytes = bits_functions::uint16_to_bytes(quotient);
        std::vector<std::byte> remainder_bytes = bits_functions::uint16_to_bytes(remainder);

        return {quotient_bytes{0}, remainder_bytes{0}};
    }
//
//    std::map<size_t, std::vector<std::vector<std::byte>>> GaloisField::find_irreducible_polynomials() {
//        std::map<size_t, std::vector<std::vector<std::byte>>> polynomials = {{1, {{std::byte{0b01000000}, std::byte{0x00}},
...
//                std::vector<std::byte> polynomial = bits_functions::uint16_to_bytes(i);
//
//                for (size_t k = 1; k <= d / 2; ++k) {
//                    auto& Q = polynomials[k];
//                    for (size_t j = 0; j < Q.size(); ++j) {
//                        if (GaloisField::divide(polynomial, Q[j]).second == std::vector{std::byte{0x00}, std::byte{0x00}} ) {
...
//
//                if (is_irreducible) {
//                    if (!polynomials.contains(d))
//                        polynomials[d] = {};
//                    polynomials[d].push_back(polynomial);
//                }
//            }
//        }
//
...
//
//        for (const auto& [degree, polynomials]: dict) {
...
//            auto a_bit = bits_functions::get_eldest_bit(a, i);
//            for (size_t j = 0; j < 8; ++j) {
//                auto b_bit = bits_functions::get_eldest_bit(b, j);
...
//    }
//
//    bool GaloisField::is_polynom_irreducible(const std::vector<std::byte> &polynomial) {
